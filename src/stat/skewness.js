...
/**
 * This class calculates and returns the skewness of a data set using the Pearson
 * method.
 *
 * <br><br>
 *
 * From wikipedia: in probability theory and statistics, skewness is a measure
 * of the asymmetry of the probability distribution of a real-valued random
 * variable about its mean. The skewness value can be positive or negative, or
 * even undefined. The qualitative interpretation of the skew is complicated.
 * For a unimodal distribution, negative skew indicates that the tail on the left
 * side of the probability density function is longer or fatter than the right
 * side – it does not distinguish these two kinds of shape. Conversely, positive
 * skew indicates that the tail on the right side is longer or fatter than the
 * left side. In cases where one tail is long but the other tail is fat, skewness
 * does not obey a simple rule.
 *
 * <br><br>
 *
 * <a href="https://en.wikipedia.org/wiki/Skewness" target="_blank">https://en.wikipedia.org/wiki/Skewness</a>
 *
 * <br><br>
 *
 * <b>Example:</b>
 ...
 * <br><br>
 *
 * <span style="color: purple;">var</span> skewness = <span style="color: purple;">new</span> Skewness();<br>
 * <span style="color: purple;">var</span> array = [ 1, 5, 6, 12, 33, 98, 22, 3, 7, 8];<br>
 * skewness.<span style="color: Sienna;">calculate</span>(array) <span style="color: purple;">  &nbsp=>&nbsp  </span> 2.584965268354471;
 *
 * <br><br>
 *
...
Skewness.prototype = new DescriptiveStatistic();
Skewness.prototype.constructor = Skewness;

/*---------------------------------- Exports ---------------------------------*/

module.exports = new Skewness();

/*---------------------------- Calculate Function ----------------------------*/

/**
 * Returns the skewness for the data set.
 * @param {number[]} data - the data set
 * @returns {number} the skewness
 */
Skewness.prototype.calculate = function(data) {
   ...
   stdev = stdev / (data.length - 1);
   stdev = Math.sqrt(stdev);

   var thirdMoment = 0;

   for(var value of data) {
      thirdMoment += Math.pow(value - mean, 3);
   }
